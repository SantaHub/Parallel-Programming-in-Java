...
//        throw new UnsupportedOperationException();
        double averagAge =  Stream.of(studentArray).parallel()
                .filter(student -> student.checkIsCurrent())
                .mapToDouble(activeStudents -> activeStudents.getAge())
                .average().getAsDouble();
        return averagAge;
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
//        throw new UnsupportedOperationException();
        String commonFirstName = Stream.of(studentArray).parallel()
                .filter(student -> !student.checkIsCurrent())
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
//        throw new UnsupportedOperationException();
        long failedAbove20Students = Stream.of(studentArray).parallel()
                .filter(student -> student.getAge() > 20 && student.getGrade() < 65)
                .count();
        return (int)failedAbove20Students;
    }
}
